...
\begin{document}
	...
	\begin{figure}[h]
		\begin{center}
			\includegraphics[width=0.5\textwidth]{Resources/Images/EV3ProgrammingSoftware.jpg}
    ...
    The color sensor will be attached to the end of an extending arm, which will be moved by a rack and worm gear to ensure the sensor can be accurately placed above the correct facelet. The worm gears will be driven by the medium sized motor, which produces less torque than the large motors but has a larger maximum rotational velocity - making it ideal for overcoming the speed reduction of the worm gears. The color sensor will be in a fixed position relative to the arm, and will be centred on the Cube's x-axis. In order to scan a full Cube, the sequence $X X X Y X X X$ will be followed, and the Up side scanned everytime a new face is seen as shown by algorithms \ref{alg:scancube} and \ref{alg:scanupface}.
    
    \begin{figure}[h]
    	\begin{center}
    		\includegraphics[width=0.5\textwidth]{Resources/Images/ColorSensor.png}
    		\caption{The color sensor mounting arm is driven by a set of worm gears}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			\Function{ScanUpFace}{}
				\State $faceletList\gets [\;]$\Comment{Initialise as empty list}
				
				\For{$i \gets 0$ \textbf{to} $4$}\Comment{4 Corner-Edge pairs}
					\State \Call{MoveColorSensor}{edgeCubie}
					\State $faceletList\gets colorValue$
					\State \Call{RotateCradle}{45}
					\State
					\State \Call{MoveColorSensor}{cornerCubie}
					\State $faceletList\gets colorValue$
					\State \Call{RotateCradle}{45}
				\EndFor
				\State \Call{MoveColorSensor}{centreCubie}
				\State $faceletList\gets colorValue$
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			\Function{ScanCube}{}
				\State $sides\gets [\;[\;], [\;], [\;], [\;], [\;], [\;]\;]$\Comment{Initialise as 6 empty lists}
				\For{$i \gets 0$ \textbf{to} \Call{Length}{sides}}
					\State $sides[i] \gets$ \Call{ScanUpFace}{}
					\If{$i == 3$} 
						\State \Call{RotateCradle}{90}\Comment{Allows Left/Right sides to be scanned after $X$-Move}
					\EndIf
					\If{$i < 5$} 
						\State \Call{XMoveCube}{}\Comment{$X$-Move the Cube to get a new Color on the Up-Side}
					\EndIf
					\If{$i == 4$} 
						\State \Call{XMoveCube}{}\Comment{Extra $X$-Move to get to Down-Side}
					\EndIf
				\EndFor
	...
	When the entire Cube is rotated, the \textbf{color} of the sides will change but the sides' orientations will not (analogous to when one turns on the spot: left and right have changed, but east and west have not). This means that the frame of reference for the moves will have to remain invariant throughout a sequence by compensating for any full Cube rotations. This is best explained with an example: the first step in performing the sequence $U\;F$ would be to do the sequence $X2\;D$ in order to complete the $U$ move; but now the original $F$ face has moved (due to the $X2$) - it must be located, the Cube must be rotated to get the original $F$ face into the cradle, and then the sequence can be completed. The simplest method of overcoming this complication is to translate the original move sequence to a sequence of color-based moves: $U\;F\;\rightarrow\;W\;G$.

    The first step of translation is to turn the move sequence into a sequence of moves dictated by the color of the face. 
